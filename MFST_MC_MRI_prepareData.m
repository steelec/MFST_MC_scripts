function PPs=MFST_MC_MRI_prepareData(dataDir,PPs,IDs,days,blocks,beforeTrialDelay,stimDuration,scoringWindowOffset,repeatedSequenceID,suppressOutput,CONV)
...
for ID=1:length(IDs) %loop on each individual
    ...
    for day=1:days %loop on each day
        ...
        for block=1:blocks %loop on each block
            ...
            [bacc, blag1, blag2, bdur, bvel, biti] = scoreMFST_MC_MRI([dataDir '/' fileName],[dataDir '/' outName], presentedSequences, scoringWindowOffset, beforeTrialDelay, stimDuration,suppressOutput,CONV);

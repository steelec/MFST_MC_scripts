...
for (ID = 1:length(IDs))
    ...
    % ACCURACY
    cmd = ['accs = PPs.' IDs{ID} '.results.d' num2str(Days(day)) '.acc;'];
    eval(cmd);
    accs = cell2mat(accs);
    % :: WE ONLY WANT 1's AND 0's FOR ACC
    accmask = ones(size(accs));
    accs = accs==accmask;
    ...
    accLRN = zeros(size(accs));
    accRND = zeros(size(accs));
...
%        hist(tempRND(~isnan(tempRND)));
        ...
        for (trial = 1:size(lags,1))
            ...
            if (LRNMask(trial) == 1)
                ...
                % Accuracy
                accLRN(trial,:) = (accs(trial,:));
                accRND(trial,:) = (accs(trial,:)).*NaN;
            ...
            % ELSE do the opposite: ADD to RND and invalidate LRN
                ...
                % Accuracy
                accRND(trial,:) = (accs(trial,:));
                accLRN(trial,:) = (accs(trial,:)).*NaN;
            %ENDIF
            end       
    ...
    % CALCULATE MEANS (BY DAY) FOR LRN AND RND
    ...
    % MEAN ACCURACY FOR TODAY
    dayAccLRN = nanmean(accLRN);
    dayAccRND = nanmean(accRND);
    ...
    % ORGANIZE CHUNK DATA FOR PLOTTING
    ...
    % ORGANIZE ACC DATA FOR PLOTTING
    accMean(ID,day) = nanmean(dayAccLRN);

function [acc, lag1, lag2, dur, vel, iti] = scoreAdaptiveMFST(in_file, out_file, in_sequences, in_windowOffset, beforeTrialDelay, stimDuration,suppressOutput,CONV)
...
while (row <= size(data,1)) %loop for every trial
    ...
    % there are no responseOffsets in this data set (b/c new Oxygen25
    % MidiKB does not report Offs in a way that the standard program can
    % read, so correct by adding a fixed value to the response to make the
    % rest of the scripts work. NOTE: lag2 and dur variables will be
    % NONSENSE!
    if isempty(responseOffsets)
        responseOffsets=responseTimes+stimDuration; %we just add the regular duration so that the rest of the code does not break
        if row==9 %only print if we are looking at the first set of responses in the block
            fprintf('\n ****** All response durations have been set to stim duration for this subject! Make sure that this is not an error and do not analyse lag2 or dur. *****\n');
        end
    end
...
%     % the following is now unecessary because keyOff is not being reported
%     % by the new Oxygen keyboards (Chris Steele Jan 7, 2013)
%     
